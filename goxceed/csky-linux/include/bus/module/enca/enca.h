...
EncaEncoding  enca_analyse                    (EncaAnalyser analyser, unsigned char *buffer, size_t size);
EncaEncoding  enca_analyse_const              (EncaAnalyser analyser, const unsigned char *buffer, size_t size);
int           enca_double_utf8_check          (EncaAnalyser analyser, const unsigned char *buffer, size_t size);
int*          enca_double_utf8_get_candidates (EncaAnalyser analyser);
int           enca_errno                      (EncaAnalyser analyser);
const char*   enca_strerror                   (EncaAnalyser analyser, int errnum);
...
#define enca_charset_is_known(cs)     ((cs) != ENCA_CS_UNKNOWN)
#define enca_charset_is_7bit(cs)      (enca_charset_properties(cs) & ENCA_CHARSET_7BIT)
#define enca_charset_is_8bit(cs)      (enca_charset_properties(cs) & ENCA_CHARSET_8BIT)
#define enca_charset_is_16bit(cs)     (enca_charset_properties(cs) & ENCA_CHARSET_16BIT)
#define enca_charset_is_32bit(cs)     (enca_charset_properties(cs) & ENCA_CHARSET_32BIT)
#define enca_charset_is_fixed(cs)     (enca_charset_properties(cs) & ENCA_CHARSET_FIXED)
#define enca_charset_is_variable(cs)  (enca_charset_properties(cs) & ENCA_CHARSET_VARIABLE)
#define enca_charset_is_binary(cs)    (enca_charset_properties(cs) & ENCA_CHARSET_BINARY)
#define enca_charset_is_regular(cs)   (enca_charset_properties(cs) & ENCA_CHARSET_REGULAR)
#define enca_charset_is_multibyte(cs) (enca_charset_properties(cs) & ENCA_CHARSET_MULTIBYTE)
...
const unsigned char* xtable(int from_charset);
void  xdata_free(void);
int parse_arg_x(const char *s);
